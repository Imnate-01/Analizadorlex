...
class Lexer:
    ...
    def scan_token(self):
        ...
        if c == '(':
            ...
            if self.match('/'):
                ...
                while True:
                    if self.is_at_end():
                        self.add_token(TipoToken.ERROR, f"Comentario multilínea no cerrado en línea {self.linea}, columna {self.columna}")
                    return
    ...
    def string(self):
        ...
        if self.is_at_end():
            self.add_token(TipoToken.ERROR, f"Cadena no cerrada en línea {self.linea}, columna {self.columna}")
        return
    ...
    # Identificar el slash como operador
    def slash(self):
        lexema = self.advance()
        self.add_token(TipoToken.SLASH, lexema)

    # Identificar el plus como operador
    def plus(self):
        lexema = self.advance()
