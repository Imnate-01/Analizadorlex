...
class Lexer:
    ...
    def scan_token(self):
        ...
        if c == '(':
            ...
            if self.match('/'):
                ...
                while True:
                    if self.is_at_end():
                        self.add_token(TipoToken.ERROR, 'Comentario multilínea no cerrado')
                        return
            ...
            self.add_token(TipoToken.ERROR, f"Caracter inesperado '{c}' en línea {self.linea}, columna {self.columna}")
    ...
    def string(self):
        ...
        if self.is_at_end():
            self.add_token(TipoToken.ERROR, "Cadena no cerrada")
    ...
    def number(self):
        ...
        if has_decimal and (self.peek() in ('e', 'E')):
            self.advance()
            if self.peek() in ('+', '-'):
                self.advance()
            while self.peek().isdigit():
        ...
        try:
            if has_decimal or 'e' in lexema or 'E' in lexema:
